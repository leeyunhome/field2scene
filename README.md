...
입력 프레임(190장 중 24장 균등 샘플, 좌→우 초반 클로즈업에서 후반 통로 전경으로 이어짐) → COLMAP 스파스 포인트클라우드(20,641점, 3DGS 학습 전 단계의 중간 산출물).

![입력 프레임 + COLMAP 포인트클라우드](assets/p1_pointcloud_with_frames.jpg)

같은 포인트클라우드를 여러 각도로 본 모습 — 좌상: 온실 골조·통로 전체 윤곽 / 우상: COLMAP GUI 원본 화면(빨간 카메라 프러스텀 190장) / 좌하: 위에서 내려다본 클로즈업(작물 열 + 카메라 궤적) / 우하: 눈높이 시점.

<p float="left">
  <img src="assets/p1_pointcloud_wide.jpg" width="49%" />
  <img src="assets/p1_pointcloud_colmap.jpg" width="49%" />
</p>
<p float="left">
  <img src="assets/p1_pointcloud.jpg" width="49%" />
